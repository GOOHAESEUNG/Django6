...
from django.contrib.auth import authenticate  #장고의 기본 authenticate 함수, 우리가 설정한 defaultauthbackend인 tokenauth 방식으로 유저를 인증해줌
...
class RegisterSerializer(serializers.ModelSerializer): #회원 가입 시리얼 라이저
    ...
    def validate(self, data): #추가적으로 비밀번호 일치 여부를 확인
        if data['password'] != data['password2']:
            raise serializers.ValidationError(
                {"password": "Password fields didn't match."}) #불일치하면 오류 메세지 보냄
...
class ProfileSerializer(serializers.ModelSerializer):
    class Meta:
        model = Profile
        fields = ("nickname", "position", "subjects","mbti","role","image")
